...
function CalidadModal({players,equipos,ligas,coaches,tempCoach,palmares,onClose,onGoToPlayer,onGoToTeam,onGoToLeague,onGoToCoach,onReload,isAdmin,setPlayers,setEquipos,setLigas,setCoaches,setTempCoach}){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const extCol=["L007","L022","L098"].includes(carrLiga)?"id_lfb":(isFeb?"id_ext":"id_espn");
  ...
  async function carrAnalizar(){
    ...
    try{
      const isFeb=["L001","L002","L003","L017","L074"].includes(carrLiga);
      if(["L007","L022","L098"].includes(carrLiga)||isFeb){
        // LFB/FEB: no hay mapear-roster; construimos roster desde temporadas + jugadoras.{id_lfb|id_ext}
        const extField=["L007","L022","L098"].includes(carrLiga)?"id_lfb":"id_ext";
        ...
        setCarrInfo({equipo:eqRow?.nombre||carrEquipoId,bd_total:roster.length,espn_total:conExt,ya_con_espn:conExt,mapeados:0,roster:roster});
      } else {
  ...
  async function ligaAnalizar(){
    ...
    for(var i=0;i<carrEquiposLiga.length;i++){
      ...
      try{
        ...
        if(["L007","L022","L098"].includes(carrLiga)||isFebL){
          const extField=["L007","L022","L098"].includes(carrLiga)?"id_lfb":"id_ext";
          ...
          const conExt=roster.filter(function(r){return r.id_espn;}).length;
          out.push({id_equipo:eq.id_equipo,equipo:eq.nombre,bd_total:roster.length,espn_total:conExt,mapeados:0,solo_en_espn_obj:[],roster:roster});
        } else {
