...
const SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF2dHhxY2t1b2xhY3ZudnJ2eXN1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzgyMzQ3OTYsImV4cCI6MjA5MzgxMDc5Nn0.0B93gvnlkGPTstRQKskzvUQOdDHeQ1vr2dwS97lhCjQ";
...
const TIPO_LABELS = {
  liga: "Liga",
  copacont: "Copa Continental",
  copadom: "Copa Nacional",
};
const TIPO_COLORS = {
  liga: ["#dbeafe", "#1d4ed8"],
...
const calcAge = (d) =>
  d
    ? Math.floor((Date.now() - new Date(d)) / (365.25 * 24 * 3600 * 1000))
    : null;
...
function FlagImg({ country }) {
  ...
  return (
    <img
      src={`https://flagpedia.net/data/flags/w160/${code}.webp`}
      width={20}
      height={13}
      alt={country}
      style={{
...
const POS_C = {
  Base: ["#dbeafe", "#1d4ed8"],
  Escolta: ["#dcfce7", "#15803d"],
  Alero: ["#fef9c3", "#a16207"],
  "Ala-Pívot": ["#ffedd5", "#c2410c"],
  Pívot: ["#fee2e2", "#b91c1c"],
};
const posStyle = (p) => {
  const [bg, color] = POS_C[p] || ["#f1f5f9", "#475569"];
  return {
    background: bg,
    color,
    fontSize: "11px",
...
function EmptyState({ icon, text, sub }) {
  return (
    ...
    >
      ...
      <div style={{ fontWeight: 600, color: "#64748b", fontSize: "16px" }}>
        {text}
      </div>
    ...
    <div>
      ...
      <Fld label="Nombre *">
        <input
          ...
          value={f.nombre}
          onChange={set("nombre")}
          placeholder="Ej: Claudia Soriano"
        />
      </Fld>
      <div
      ...
      >
        <Fld label="Posición">
          <select style={inp} value={f.posicion} onChange={set("posicion")}>
            {POSITIONS.map((p) => (
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "8px" }}>
        ...
        <button
          onClick={() => f.nombre.trim() && onSave(f)}
          disabled={saving || !f.nombre.trim()}
          style={{
            flex: 1,
            background: f.nombre.trim() ? "#f97316" : "#fed7aa",
            color: "#fff",
            border: "none",
            borderRadius: "10px",
            padding: "11px",
            cursor: f.nombre.trim() ? "pointer" : "not-allowed",
            fontWeight: 700,
          }}
        >
    ...
    <div>
      <Fld label="Temporada *">
        <input
          style={inp}
          value={f.temporada}
          onChange={(e) => setF((p) => ({ ...p, temporada: e.target.value }))}
          placeholder="2024-25"
        />
      </Fld>
      <Fld label="Equipo *">
        <select
          style={inp}
          value={f.id_equipo}
          onChange={(e) => setF((p) => ({ ...p, id_equipo: e.target.value }))}
        >
      ...
      <Fld label="Competición *">
        <select
          style={inp}
          value={f.id_liga}
          onChange={(e) => setF((p) => ({ ...p, id_liga: e.target.value }))}
        >
...
function PlayersView({
  ...
  const ligaMap = useMemo(() => {
    const m = {};
    ligas.forEach((l) => (m[l.id_liga] = l));
    return m;
  }, [ligas]);
  ...
  const playerTipos = useMemo(() => {
    if (!selected) return [];
    const seen = new Set();
    sortS(selected.seasons).forEach((s) => {
      const t = ligaMap[s.id_liga]?.tipo;
  ...
  const defaultTipo = useMemo(() => {
    ...
    const tipos = new Set(
      sortS(selected.seasons)
        .map((s) => ligaMap[s.id_liga]?.tipo)
        .filter(Boolean)
    );
    return tipos.has("liga")
      ? "liga"
      : ligaMap[sortS(selected.seasons)[0]?.id_liga]?.tipo || null;
  }, [selected, ligaMap]);
  ...
  const filteredSeasons = useMemo(() => {
    ...
    return currentTipo
      ? all.filter((s) => ligaMap[s.id_liga]?.tipo === currentTipo)
      : all;
    ...
    .filter((p) => {
      ...
      return (
        (!q ||
          ...
          p.id_jugadora?.toLowerCase().includes(q) ||
          p.nacionalidad?.toLowerCase().includes(q) ||
          p.seasons?.some((s) =>
            equipoMap[s.id_equipo]?.nombre?.toLowerCase().includes(q)
          )) &&
  ...
  const addPlayer = async (f) => {
    ...
    try {
      const { data } = await supabase
        .from("jugadoras")
        .select("id_jugadora")
        .order("id_jugadora", { ascending: false })
        .limit(1);
      const newId = `J${String(
        parseInt((data?.[0]?.id_jugadora || "J000").slice(1)) + 1
      ).padStart(3, "0")}`;
      await supabase.from("jugadoras").insert({
        id_jugadora: newId,
        nombre: f.nombre,
        posicion: f.posicion,
        nacionalidad: f.nacionalidad,
        fecha_nac: f.fecha_nac || null,
        altura_cm: f.altura_cm ? parseInt(f.altura_cm) : null,
        foto: f.foto || null,
      });
      await onReload();
      setModal(null);
  ...
  const delPlayer = async () => {
    try {
      await supabase.from("temporadas").delete().eq("id_jugadora", selId);
  ...
  if (players.length === 0)
    return (
      <EmptyState
        icon="👩‍🏀"
        text="No hay jugadoras"
        sub="Verifica la conexión con Supabase"
      />
    );
  ...
  if (selected)
    return (
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "20px" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                {selected.posicion && (
                  <span style={posStyle(selected.posicion)}>
                    {selected.posicion}
                  </span>
                )}
                {selected.nacionalidad && (
                  ...
                  >
                    <FlagImg country={selected.nacionalidad} />
                    {selected.nacionalidad}
                  </span>
                )}
              </div>
                ...
                {selected.altura_cm && (
                  <div style={{ fontSize: "13px" }}>
                    ...
                    <span style={{ fontWeight: 600, color: "#334155" }}>
                      {selected.altura_cm} cm
                    </span>
                  </div>
                )}
                {selected.fecha_nac && (
                  <div style={{ fontSize: "13px" }}>
                    <span style={{ color: "#94a3b8" }}>Edad: </span>
                    <span style={{ fontWeight: 600, color: "#334155" }}>
                      {calcAge(selected.fecha_nac)} años
                    </span>
                  </div>
                )}
              </div>
        ...
        >
          ...
          >
            ...
            >
              Historial{" "}
              <span
                style={{ color: "#94a3b8", fontWeight: 400, fontSize: "14px" }}
              >
                ({selected.seasons.length})
              </span>
            </h2>
            <button
              onClick={() => setModal("addSeason")}
              style={{
                background: "#f97316",
                color: "#fff",
                border: "none",
          ...
          {filteredSeasons.length === 0 ? (
            ...
            >
              Sin temporadas para esta competición
            </div>
          ) : (
                      ...
                      >
                        ...
                        >
                          <div
                            ...
                            onClick={() => onGoToTeam(s.id_equipo)}
                          >
                            ...
                            <div>
                              ...
                              >
                                {eq?.pais && <FlagImg country={eq.pais} />}
                                {lig?.nombre || s.id_liga}
                              </div>
                            </div>
                          </div>
        ...
        {modal === "editPlayer" && (
          <Modal title="Editar jugadora" onClose={() => setModal(null)}>
            <PlayerForm
              initial={{ ...selected }}
              onSave={updPlayer}
        ...
        {modal === "addSeason" && (
          <Modal title="Nueva temporada" onClose={() => setModal(null)}>
            <SeasonForm
              equipos={equipos}
              ligas={ligas}
              onSave={addSeason}
        ...
        {modal === "editSeason" && (
          <Modal title="Editar temporada" onClose={() => setModal(null)}>
            <SeasonForm
              initial={editSeason}
              equipos={equipos}
              ligas={ligas}
        ...
        {del && (
          <ConfirmDel
            msg={
              del.type === "player"
                ? "Se eliminará la jugadora y todo su historial."
                : "Se eliminará esta temporada."
            }
            onCancel={() => setDel(null)}
            onConfirm={() =>
              del.type === "player" ? delPlayer() : delSeason(del.id)
            }
          />
        )}
      </div>
    ...
    <div style={{ maxWidth: "880px", margin: "0 auto", padding: "20px" }}>
      ...
      >
        {[
          ...
          [
            "📋",
            players.reduce((n, p) => n + (p.seasons?.length || 0), 0),
            "Temporadas",
          ],
          [
            "🌍",
            new Set(
              players.flatMap((p) => (p.seasons || []).map((s) => s.id_equipo))
            ).size,
            "Equipos únicos",
          ],
        ].map(([e, v, l]) => (
          ...
          >
            ...
            <div
              style={{ fontSize: "24px", fontWeight: 800, color: "#1e293b" }}
            >
              {v}
            </div>
      ...
      <div style={{ fontSize: "13px", color: "#94a3b8", marginBottom: "12px" }}>
        {filtered.length} jugadora{filtered.length !== 1 ? "s" : ""}
      </div>
      <div
        style={{
          display: "grid",
      ...
      >
        {filtered.map((p) => {
          ...
          const last =
            allS.find((s) => ligaMap[s.id_liga]?.tipo === "liga") || allS[0];
          const lastEq = last ? equipoMap[last.id_equipo] : null;
          return (
            <div
              key={p.id_jugadora}
              onClick={() => {
                setSelId(p.id_jugadora);
                setActiveTipo(null);
              }}
              style={{
                background: "#fff",
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    <FlagImg country={p.nacionalidad || ""} />
                    {p.nacionalidad || "—"}
                    {p.altura_cm ? ` · ${p.altura_cm} cm` : ""}
                  </div>
                </div>
                {p.posicion && (
                  <span style={posStyle(p.posicion)}>{p.posicion}</span>
                )}
              </div>
              <div
                  ...
                  <>
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          {last.temporada} · {lastEq.nombre}
                        </div>
                        <div style={{ fontSize: "11px", color: "#94a3b8" }}>
                          {ligaMap[last.id_liga]?.nombre || last.id_liga}
                        </div>
                      </div>
                    </div>
                  </>
      ...
      {modal === "addPlayer" && (
        <Modal title="Nueva jugadora" onClose={() => setModal(null)}>
          <PlayerForm
            onSave={addPlayer}
            onCancel={() => setModal(null)}
            saving={saving}
...
function TeamsView({
  ...
  const teamIndex = useMemo(() => {
    const map = {};
    equipos.forEach((e) => {
      map[e.id_equipo] = { eq: e, years: new Set(), players: [] };
    });
    players.forEach((p) => {
      (p.seasons || []).forEach((s) => {
        if (!map[s.id_equipo])
          map[s.id_equipo] = {
            eq: equipoMap[s.id_equipo] || {
    ...
    return Object.values(map).sort((a, b) =>
      (a.eq.nombre || "").localeCompare(b.eq.nombre || "")
    );
  ...
  const allSeasons = useMemo(
    () =>
      [
        ...new Set(
          players.flatMap((p) =>
            (p.seasons || []).map((s) => s.temporada || "").filter(Boolean)
          )
        ),
  ...
  const allLeagues = [
    ...new Set(ligas.map((l) => l.nombre).filter(Boolean)),
  ].sort();
  ...
  const filtered = teamIndex.filter(({ eq, years, players: pl }) => {
    const matchSearch =
      ...
      eq.id_equipo?.toLowerCase().includes(search.toLowerCase());
    const matchLeague =
      !filterLeague ||
      pl.some(({ season }) => ligaMap[season.id_liga]?.nombre === filterLeague);
    const matchSeason = !filterSeason || years.has(filterSeason);
    return matchSearch && matchLeague && matchSeason;
  });

  const selected = selId
    ? teamIndex.find((t) => t.eq.id_equipo === selId)
    : null;
  const years = selected
    ? [...selected.years].sort((a, b) => b.localeCompare(a))
    : [];
    ...
    ? [
        ...new Map(
          selected.players
            .filter(
              ({ season }) =>
                !effectiveYear || season.temporada === effectiveYear
            )
            .map(({ player, season }) => [
              player.id_jugadora,
              { player, season },
        ...
        >
          ...
          >
            {[
              ...
              [
                new Set(
                  selected.players.map(({ player }) => player.id_jugadora)
                ).size,
                "Jugadoras únicas",
              ],
              [selected.players.length, "Apariciones"],
            ].map(([v, l]) => (
              <div
                key={l}
                style={{
        ...
        >
          ...
          >
            ...
            {years.length > 0 && (
              <select
                value={effectiveYear || ""}
                onChange={(e) => setSelYear(e.target.value || null)}
                style={{
                  border: "1.5px solid #e2e8f0",
                  borderRadius: "10px",
                  padding: "8px 14px",
          ...
          {squad.length === 0 ? (
            ...
            >
              Sin jugadoras para esta temporada
            </div>
          ) : (
            <div
            ...
            >
              {squad.map(({ player }, i) => (
                <div
                  key={i}
                  onClick={() => onGoToPlayer(player.id_jugadora)}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "12px",
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      <FlagImg country={player.nacionalidad || ""} />
                      {player.nacionalidad || "—"}
                      {player.altura_cm ? ` · ${player.altura_cm} cm` : ""}
                    </div>
                  </div>
                  <span style={posStyle(player.posicion || "")}>
                    {player.posicion || "—"}
                  </span>
                </div>
              ))}
...
function LeaguesView({ ligas, players, equipos, onGoToTeam }) {
  ...
  const { years, teamsByYear } = useMemo(() => {
    ...
    return {
      years: Object.keys(yMap).sort((a, b) => b.localeCompare(a)),
      teamsByYear: yMap,
    };
  }, [selected, players]);
  ...
  const teamIds = effectiveYear
    ? [...(teamsByYear[effectiveYear] || new Set())]
    : [];
  const teamsInLeague = teamIds
    .map((id) => equipoMap[id])
    .filter(Boolean)
    .sort((a, b) => a.nombre.localeCompare(b.nombre));
  ...
  const playersInYear = useMemo(() => {
    ...
    players.forEach((p) =>
      (p.seasons || []).forEach((t) => {
        if (t.id_liga === selected.id_liga && t.temporada === effectiveYear)
          s.add(p.id_jugadora);
      })
    );
    return s;
  ...
  const filtered = ligas.filter(
    (l) => !search || l.nombre?.toLowerCase().includes(search.toLowerCase())
  );
  const ligasByTipo = useMemo(() => {
    const g = { liga: [], copacont: [], copadom: [], other: [] };
    filtered.forEach((l) => {
      if (g[l.tipo]) g[l.tipo].push(l);
      else g.other.push(l);
    });
    return g;
  }, [filtered]);
    ...
    return (
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "20px" }}>
        ...
        >
          ...
          >
            {[
              [years.length, "Temporadas registradas"],
              [teamsInLeague.length, `Equipos ${effectiveYear || ""}`],
              [playersInYear.size, `Jugadoras ${effectiveYear || ""}`],
            ].map(([v, l]) => (
              <div
                key={l}
                style={{
        ...
        >
          ...
          {teamsInLeague.length === 0 ? (
            ...
            >
              ...
              <div>Sin equipos para esta temporada</div>
            </div>
          ) : (
            <div
              style={{
            ...
            >
              {teamsInLeague.map((eq) => (
                <div
                  key={eq.id_equipo}
                  onClick={() => onGoToTeam(eq.id_equipo)}
                  style={{
                    background: "#f8fafc",
                    borderRadius: "14px",
                    padding: "14px",
          ...
          <div key={tipo} style={{ marginBottom: "24px" }}>
            ...
            >
              ...
              <span style={{ fontSize: "12px", color: "#94a3b8" }}>
                {items.length} competición{items.length !== 1 ? "es" : ""}
              </span>
            </div>
            <div
              style={{
            ...
            >
              {items.map((l) => {
                ...
                players.forEach((p) =>
                  (p.seasons || []).forEach((s) => {
                    if (s.id_liga === l.id_liga) {
                      teamSet.add(s.id_equipo);
                      yearSet.add(s.temporada);
                    }
                  })
                );
                return (
...
export default function App() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const sbp = {};
      (rT.data || []).forEach((t) => {
        if (!sbp[t.id_jugadora]) sbp[t.id_jugadora] = [];
        sbp[t.id_jugadora].push(t);
      });
      setPlayers(
        (rJ.data || []).map((j) => ({
          ...j,
          seasons: sbp[j.id_jugadora] || [],
        }))
      );
      setEquipos(rE.data || []);
      setLigas(rL.data || []);
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center", color: "#94a3b8" }}>
          ...
          <div style={{ fontWeight: 600 }}>Cargando desde Supabase...</div>
        </div>
      </div>
    );
      ...
      <div style={{ paddingTop: "8px" }}>
        {tab === "jugadoras" && (
          <PlayersView
            ...
            onClearPlayer={() => setOpenPlayerId(null)}
          />
        )}
        {tab === "equipos" && (
          <TeamsView
            ...
            onClearTeam={() => setOpenTeamId(null)}
          />
        )}
        {tab === "ligas" && (
          <LeaguesView
